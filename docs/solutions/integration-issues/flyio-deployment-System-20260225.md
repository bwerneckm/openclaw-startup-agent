...
severity: high
tags: [fly-io, whatsapp, docker, deployment, nginx, cors]
---

# Troubleshooting: Fly.io Deployment

## Problem
Deploying to Fly.io with custom skills required solving a chain of configuration issues: missing Dockerfile, gateway bind address, WhatsApp pairing failures from wrong region, and Control UI origin restrictions.

## Environment
- Module: System (Fly.io deployment)
...
**Attempted Solution 1:** Building from `node:22-bookworm` with `npm install`
- **Why it failed:** The npm package doesn't include the full gateway entrypoint, nginx reverse proxy, or Control UI assets that the Docker base image provides.

**Attempted Solution 2:** Using `--bind lan` in gateway startup
- **Why it failed:** Fly.io's proxy needs the app to listen on `0.0.0.0`, not the LAN interface. The Docker image uses nginx on port 8080 as a reverse proxy to the gateway on `127.0.0.1:18789`, so the bind flag on the gateway itself is irrelevant.
...
### 1. Use a proper Docker base image (not npm install)
...
The entrypoint starts nginx on `${PORT:-8080}` which proxies to the gateway. Do NOT set a `[processes]` block — let the container's own entrypoint handle startup.
...
### 4. Add allowedOrigins to gateway config

SSH into the machine and update the gateway config:
...
```bash
fly secrets set GATEWAY_TOKEN=$(openssl rand -hex 32) --app your-app
fly secrets set ANTHROPIC_API_KEY=sk-ant-... --app your-app
```

The entrypoint script **exits immediately** if either the gateway token or at least one AI provider API key is missing.

## Why This Works

1. **Base image**: The Docker base image includes the full entrypoint with nginx reverse proxy, configure.js, and Control UI assets. The npm package alone is insufficient.
...
- Use a proper Docker base image for deployments, not just `npm install`
- Match Fly.io region to phone number country (GRU for Brazil, IAD for US, etc.)
- Set `allowedOrigins` in the gateway config for the Fly.io domain before first access
